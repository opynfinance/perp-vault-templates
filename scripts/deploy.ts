...
async function deployContracts() {
    ...
    //     "StakeDAO ETH-frax Put Strategy",
    //     "sdETH-FraxPut", {gasPrice: 108000000000}
    // ) as OpynPerpVault;
    const opynPerpVault = (await ethers.getContractAt('OpynPerpVault', '0x31F3bDaaE5df0a1cafb57A9addcBa7869185706f')) as OpynPerpVault
    ...
    const shortOTokenActionWithSwap = (await ethers.getContractAt('ShortOTokenActionWithSwap', '0x7e35a74dCF3227a65A479F3269840644Df68924C')) as ShortOTokenActionWithSwap
...
async function setPerpVaultStrategy({
    ...
    // await opynPerpVault.connect(deployer).setActions([shortOTokenActionWithSwap.address], {gasPrice: 108000000000})
...
async function setupOwnership({
    ...
    // await opynPerpVault.connect(deployer).transferOwnership(newOwnerAddress, {gasPrice: 80000000000})

    console.log(`\nOpynPerpVault ownership transferred to ${newOwnerAddress}.`)

    // transfer ShortOTokenActionWithSwap ownership
    await shortOTokenActionWithSwap.connect(deployer).transferOwnership(newOwnerAddress, {gasPrice: 1000000000000})
